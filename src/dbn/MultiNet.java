...
public class MultiNet {
	 ...
	 */
	public MultiNet(Observations o, int numClusters, boolean is_bcDBN, boolean is_cDBN, boolean spanning, int intra_ind,
			int root, int maxParents, boolean stationaryProcess, boolean multithread) {
		super();
		this.o = o;
	...
	private double[][][] computeClusters(List<DynamicBayesNet> net, boolean stationaryProcess, boolean mostProbable) {
		...
		for (int s = 0; s < numSubjects; s++) {
				...
				for (int t = 0; t < numTransitions; t++) {
					for (int n = 0; n < numAttributes; n++) {
						if (stationaryProcess) {
							probabilityAux += Math
									.log(dbn.transitionNets.get(0).getParameters(n, observations[t][s]).get(0));
						} else {
							probabilityAux += Math
									.log(dbn.transitionNets.get(t).getParameters(n, observations[t][s]).get(0));
						}
					}
				}
			...
			probabilitySum = 0;
			for (int c = 0; c < numClusters; c++) {
				probabilitySum += alpha[c] * (double) Math.ceil(newClustering[0][s][c] * 1000000000000000d)
						/ 1000000000000000d;
...
//			}
			sb.append(networks.get(c).toString(printParameters));
			sb.append("Alpha: " + alpha[c] + ls + ls);
		}
...
//		sb.append(ls + "Final Score: " + score + ls);
	...
	public static void main(String args[]) {
		...
		Option inputFile = Option.builder("i").hasArg().required(true).desc("Folder of the dataset.").argName("path")
				.longOpt("inputFile").build();
		Option numParents = Option.builder("n").longOpt("numClusters").desc("Number of clusters.").required(true)
